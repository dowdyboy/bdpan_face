...
class WithBias_LayerNorm(nn.Layer):
    ...
    def forward(self, x):
        # paddle.transpose((x - mu) / paddle.sqrt(sigma+1e-6), [0,2,3,1]) * self.weight
        x = paddle.transpose(x, [0, 2, 3, 1])
        mu = x.mean(-1, keepdim=True)
        sigma = x.var(-1, keepdim=True, unbiased=False)
        ret = (x - mu) / paddle.sqrt(sigma+1e-6) * self.weight + self.bias
        return paddle.transpose(ret, [0, 3, 1, 2])
...
class AIDR(nn.Layer):
    ...
    def __init__(self, in_channels=3, out_channels=3, num_c=48, mid_channels=[64, 128, 256]):
        ...
        self.en_block1 = nn.Sequential(
            ...
            nn.Conv2D(in_channels, num_c, 1, padding=0, bias_attr=True),
            # nn.LeakyReLU(negative_slope=0.1),
            nn.GELU(),
            # nn.Conv2D(num_c, num_c, 3, padding=1, bias_attr=True),
            nn.Conv2D(num_c, num_c, 1, padding=0, bias_attr=True),
            # nn.LeakyReLU(negative_slope=0.1),
            nn.GELU(),
            nn.MaxPool2D(2))
        ...
        self.de_block1 = nn.Sequential(
            ...
            nn.Conv2D(num_c*2 + mid_channels[2], num_c*2, 1, padding=0, bias_attr=True),
            # nn.LeakyReLU(negative_slope=0.1),
            nn.GELU(),
            ...
            nn.Conv2D(num_c*2, num_c*2, 1, padding=0, bias_attr=True),
            # nn.LeakyReLU(negative_slope=0.1),
            nn.GELU(),
            nn.Upsample(scale_factor=2, mode='nearest'))
        ...
        self.de_block2 = nn.Sequential(
            ...
            nn.Conv2D(num_c*3 + mid_channels[1], num_c*2, 1, padding=0, bias_attr=True),
            # nn.LeakyReLU(negative_slope=0.1),
            nn.GELU(),
            # nn.Conv2D(num_c*2, num_c*2, 3, padding=1, bias_attr=True),
            nn.Conv2D(num_c*2, num_c*2, 1, padding=0, bias_attr=True),
        ...
        self.de_block3 = nn.Sequential(
            ...
            nn.Conv2D(num_c*3 + mid_channels[0], num_c*2, 1, padding=0, bias_attr=True),
            # nn.LeakyReLU(negative_slope=0.1),
            nn.GELU(),
            # nn.Conv2D(num_c*2, num_c*2, 3, padding=1, bias_attr=True),
            nn.Conv2D(num_c*2, num_c*2, 1, padding=0, bias_attr=True),
        ...
        self.de_block4 = nn.Sequential(
            ...
            nn.Conv2D(num_c*3, num_c*2, 1, padding=0, bias_attr=True),
            # nn.LeakyReLU(negative_slope=0.1),
            nn.GELU(),
            # nn.Conv2D(num_c*2, num_c*2, 3, padding=1, bias_attr=True),
            nn.Conv2D(num_c*2, num_c*2, 1, padding=0, bias_attr=True),
        ...
        self.de_block5 = nn.Sequential(
            ...
            nn.Conv2D(num_c*2 + in_channels, 64, 1, padding=0, bias_attr=True),
            # nn.LeakyReLU(negative_slope=0.1),
            nn.GELU(),
            # nn.Conv2D(64, 32, 3, padding=1, bias_attr=True),
            nn.Conv2D(64, 32, 1, padding=0, bias_attr=True),
            # nn.LeakyReLU(negative_slope=0.1),
            nn.GELU(),
            # CAPABlock(32, reduction=1, pa_first=False),
            # nn.Conv2D(32, out_channels, 3, padding=1, bias_attr=True)
            nn.Conv2D(32, out_channels, 1, padding=0, bias_attr=True)
        )
...
class Residual(nn.Layer):
    def __init__(self, in_channels, out_channels, same_shape=True, **kwargs):
        ...
        self.batch_norm2d = WithBias_LayerNorm(out_channels)
