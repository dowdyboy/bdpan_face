...
parser.add_argument('--ms', type=int, nargs='+', default=[640, 768, 896, 0], help='multi scale crop')
...
def build_data():
    train_dataset = FaceDataset(root_dir=args.data_dir, is_train=True, rate=0.99, mosic_p=1.0, )
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size,
                              shuffle=True, num_workers=args.num_workers, drop_last=True, collate_fn=merge_and_crop_bat)
    val_dataset = FaceDataset(root_dir=args.data_dir, is_train=False, rate=0.99, h_flip_p=0.0, )
